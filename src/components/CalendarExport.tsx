  ...
  const generateCalendarEvents = (): CalendarEvent[] => {
    ...
    for (let i = 0; i < data.length; i += 2) {
      ...
      days.forEach((day, dayIndex) => {
        ...
        } else if (training.toLowerCase().includes('x-train')) {
          title = 'Cross Training';
        }

        let eventDescription = `Training: ${training}\n`;
        if (description) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
      ...
      <p className="text-gray-600 mb-6">
        Export your training plan to your calendar app. This will create {eventCount} workout events.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <button
          ...
          className="flex items-center justify-center px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors font-medium"
        >
          {isExporting ? (
            <>
        ...
        <button
          ...
          disabled={isExporting}
          className="flex items-center justify-center px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors font-medium"
        >
          {isExporting ? (
            <>
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
      ...
      <div className="mt-4 p-4 bg-blue-50 rounded-lg">
        ...
        <ul className="text-sm text-blue-800 space-y-1">
          <li><strong>Google Calendar:</strong> Downloads an .ics file with import instructions</li>
          <li><strong>.ics File:</strong> Compatible with Apple Calendar, Outlook, and most calendar apps</li>
          <li><strong>Note:</strong> Rest days and empty training days are not included in the export</li>
        </ul>
      </div>
    </div>
  );
